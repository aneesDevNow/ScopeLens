...
function getPlanStyle(slug: string, index: number, isLoggedIn: boolean, currentPlanSlug: string | null, planSlugs: string[], planName: string) {
    const isPopular = slug === "professional" || index === 2;
    const isCurrentPlan = isLoggedIn && currentPlanSlug === slug;
    const currentPlanIndex = currentPlanSlug ? planSlugs.indexOf(currentPlanSlug) : -1;
    const thisPlanIndex = planSlugs.indexOf(slug);
    const isUpgrade = isLoggedIn && currentPlanIndex >= 0 && thisPlanIndex > currentPlanIndex;
    const isDowngrade = isLoggedIn && currentPlanIndex >= 0 && thisPlanIndex < currentPlanIndex;

    let buttonText: string;
    let buttonLink: string;
    if (isCurrentPlan) {
        buttonText = `Current Plan — ${planName}`;
        buttonLink = "/scan";
    } else if (!isLoggedIn) {
        buttonText = slug === "free" ? "Get Started" : "Sign Up";
        buttonLink = slug === "free" ? "/signup" : `/signup?plan=${slug}`;
    } else if (isUpgrade) {
        buttonText = `Upgrade to ${slug === "professional" ? "Pro" : slug.charAt(0).toUpperCase() + slug.slice(1)}`;
        buttonLink = `/checkout?plan=${slug}`;
    } else if (isDowngrade) {
    ...
    return {
        ...
        buttonClass: isCurrentPlan
            ? "w-full py-3.5 text-green-700 font-semibold rounded-xl border-2 border-green-300 bg-green-50 cursor-default"
            : isPopular
...
export default function PricingSection({ plans, isLoggedIn = false, currentPlanSlug = null }: { plans: Plan[]; isLoggedIn?: boolean; currentPlanSlug?: string | null }) {
            ...
            <div className="container mx-auto px-6">
                ...
                <div className={`grid gap-8 max-w-6xl mx-auto ${plans.length <= 3 ? 'md:grid-cols-3' : 'md:grid-cols-4'}`}>
                    {plans.map((plan, index) => {
                        ...
                        const style = getPlanStyle(plan.slug, index, isLoggedIn, currentPlanSlug, planSlugs, plan.name);
                        ...
                        return (
                            <div key={plan.id} className={`${style.cardClass} transition-all duration-300 ${isLoading ? "opacity-70" : "opacity-100"}`}>
                                ...
                                {style.isCurrentPlan && (
                                    <div className={`absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm font-bold rounded-full shadow-lg`}>
                                        ✓ Your Plan
                                    </div>
                                )}
                                <div className="mb-6">
                                ...
                                {style.isCurrentPlan ? (
                                    <button className={style.buttonClass} disabled>
                                        {style.buttonText}
                                    </button>
                                ) : (
