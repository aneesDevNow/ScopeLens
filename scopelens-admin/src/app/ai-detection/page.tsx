...
interface QueueItem {
    ...
    zerogpt_accounts: { label: string } | null;
    label: string;
}
...
export default function AIDetectionPage() {
    const [accounts, setAccounts] = useState<ZeroGPTAccount[]>([]);
    ...
    const [newMaxConcurrent, setNewMaxConcurrent] = useState("2");
    ...
    const fetchAccounts = useCallback(async () => {
        try {
            const res = await fetch("/api/admin/ai-detection/accounts");
    ...
    const fetchQueue = useCallback(async () => {
        try {
            const res = await fetch("/api/admin/ai-detection/queue");
            const data = await res.json();
            setQueue(data.queue || []);
    ...
    useEffect(() => {
        ...
        const autoProcessInterval = setInterval(async () => {
            ...
            // Only process if there are waiting items
            const res = await fetch("/api/admin/ai-detection/queue");
            const data = await res.json();
            ...
            try {
                await fetch("/api/admin/ai-detection/process", { method: "POST" });
                await fetchQueue();
            } catch {
    ...
    const handleAddAccount = async () => {
        ...
        try {
            const res = await fetch("/api/admin/ai-detection/accounts", {
                ...
                body: JSON.stringify({
                    label: newLabel || "Account",
                    bearer_token: newToken,
                    max_concurrent: parseInt(newMaxConcurrent) || 2,
                    max_retries: parseInt(newMaxRetries) || 3,
                }),
            });
            if (res.ok) {
                setNewLabel("");
                setNewToken("");
                setNewMaxConcurrent("2");
                setNewMaxRetries("3");
    ...
    const handleToggleActive = async (account: ZeroGPTAccount) => {
        await fetch(`/api/admin/ai-detection/accounts/${account.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    const handleDeleteAccount = async (id: string) => {
        ...
        await fetch(`/api/admin/ai-detection/accounts/${id}`, { method: "DELETE" });
        fetchAccounts();
    };
    ...
    const handleSaveEdit = async (id: string) => {
        const body: Record<string, unknown> = {
            label: editLabel,
            max_concurrent: parseInt(editMaxConcurrent) || 2,
            max_retries: parseInt(editMaxRetries) || 3,
        };
        if (editToken.trim()) {
            body.bearer_token = editToken;
        }
        await fetch(`/api/admin/ai-detection/accounts/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    const handleProcessQueue = async () => {
        ...
        try {
            ...
            while (hasMore) {
                const res = await fetch("/api/admin/ai-detection/process", { method: "POST" });
                const data = await res.json();

                totalProcessed += data.processed || 0;
                hasMore = (data.remaining || 0) > 0 && (data.processed || 0) > 0;

                // Refresh stats between batches
                await fetchQueue();
    ...
    const handleQueueAction = async (action: string) => {
        await fetch("/api/admin/ai-detection/queue", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        return (
            <div className="p-6 flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground">Loading AI Detection settings...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="p-6">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage ZeroGPT accounts and scan processing queue</p>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle>ZeroGPT Accounts</CardTitle>
                                <CardDescription>Manage API tokens for AI detection</CardDescription>
                            </div>
                    ...
                    <CardContent>
                        ...
                        {showAddForm && (
                            <div className="bg-muted/50 p-4 rounded-lg mb-4 space-y-3">
                                <Input
                                    placeholder="Account label (e.g. Account 1)"
                                    value={newLabel}
                                ...
                                <div className="flex gap-2">
                                    <Button size="sm" onClick={handleAddAccount} disabled={!newToken.trim() || addingAccount}>
                        ...
                        {accounts.length === 0 ? (
                            <div className="text-center py-8 text-muted-foreground">
                                ...
                                <p className="text-xs">Add a ZeroGPT account to start processing</p>
                            </div>
                        ) : (
                                        ...
                                        {editingId === account.id ? (
                                            ...
                                            <>
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <button
                                                            onClick={() => {
                                                                ...
                                                                setEditMaxRetries(String(account.max_retries ?? 3));
                                                                setEditToken("");
                                                            }}
                                                ...
                                                <div className="text-xs text-muted-foreground font-mono mb-2">
                                                    {maskToken(account.bearer_token)}
                                                </div>
                                                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                                    <span>Max: {account.max_concurrent} concurrent</span>
                                                    <span>Retries: {account.max_retries ?? 3}</span>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Recent scan detection requests</CardDescription>
                            </div>
                            <div className="flex gap-2">
                                {stats.completed > 0 && (
                    ...
                    <CardContent>
                        {queue.length === 0 ? (
                            <div className="text-center py-8 text-muted-foreground">
                                ...
                                <p className="text-xs">Items appear here when scans are uploaded</p>
                            </div>
                        ) : (
                                ...
                                {queue.map((item) => (
                                    <div key={item.id} className="border rounded-lg p-3">
                                        ...
                                        <div className="text-xs text-muted-foreground mt-1">
                                            ...
                                            {item.zerogpt_accounts && (
                                                <span className="ml-2">→ {item.zerogpt_accounts.label}</span>
                                            )}
                                        </div>
