...
run_sql "plans" "CREATE TABLE IF NOT EXISTS public.plans (id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY, name text NOT NULL UNIQUE, slug text NOT NULL UNIQUE, price_monthly numeric(10,2) DEFAULT 0 NOT NULL, price_yearly numeric(10,2), scans_per_day integer DEFAULT 5 NOT NULL, features jsonb DEFAULT '[]'::jsonb, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), reseller_price_monthly numeric(10,2), reseller_price_yearly numeric(10,2), reseller_discount_percent integer DEFAULT 20);"
...
echo ""
echo "=== Seeding Plans ==="
run_sql "Free plan" "INSERT INTO public.plans (name, slug, price_monthly, price_yearly, scans_per_day, features, reseller_price_monthly, reseller_price_yearly, reseller_discount_percent) VALUES ('Free', 'free', 0, 0, 1, '{\"PDF & DOCX support\": true, \"1 AI scan per day\": true, \"Standard report summary\": true, \"Basic plagiarism detection\": true}', 0, 0, 20) ON CONFLICT (name) DO NOTHING;"

run_sql "Starter plan" "INSERT INTO public.plans (name, slug, price_monthly, price_yearly, scans_per_day, features, reseller_price_monthly, reseller_price_yearly, reseller_discount_percent) VALUES ('Starter', 'starter', 12, 114, 15, '{\"Email support\": true, \"15 AI scans per day\": true, \"Detailed originality reports\": true, \"Advanced plagiarism detection\": true, \"Multi-format support (PDF, DOCX, TXT)\": true}', 8.40, 79.80, 30) ON CONFLICT (name) DO NOTHING;"

run_sql "Professional plan" "INSERT INTO public.plans (name, slug, price_monthly, price_yearly, scans_per_day, features, reseller_price_monthly, reseller_price_yearly, reseller_discount_percent) VALUES ('Professional', 'professional', 17, 161, 25, '{\"25 AI scans per day\": true, \"Deep content analysis\": true, \"Priority email support\": true, \"Export detailed PDF reports\": true, \"Sentence-level AI highlighting\": true}', 11.90, 112.70, 30) ON CONFLICT (name) DO NOTHING;"

run_sql "Institution plan" "INSERT INTO public.plans (name, slug, price_monthly, price_yearly, scans_per_day, features, reseller_price_monthly, reseller_price_yearly, reseller_discount_percent) VALUES ('Institution', 'institution', 59, 559, 100, '{\"100 AI scans per day\": true, \"Dedicated account manager\": true, \"Bulk upload & batch processing\": true, \"Full content integrity analysis\": true}', 41.30, 391.30, 30) ON CONFLICT (name) DO NOTHING;"
